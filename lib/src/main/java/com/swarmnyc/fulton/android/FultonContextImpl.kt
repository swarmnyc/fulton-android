...
class FultonContextImpl(context: Context, options: FultonInitOptions) : FultonContext {
    ...
    override var identityManager: IdentityManager = options.identityManager ?: IdentityManagerImpl(context, options)
    override var requestExecutor: RequestExecutor = options.requestExecutor ?: RequestExecutorImpl()
    override var mockRequestExecutor: RequestExecutor? = options.mockRequestExecutor
}
